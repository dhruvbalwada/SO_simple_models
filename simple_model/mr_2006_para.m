...
for char_index = 1:length(b0m)
   if char_index ==1
                dpsi_res_0db(char_index) = (psi_res_0(char_index+1)-psi_res_0(char_index))/(b0m(char_index+1)-b0m(char_index));
            elseif char_index == length(b0m)
                dpsi_res_0db(char_index) = (psi_res_0(char_index)-psi_res_0(char_index-1))/(b0m(char_index)-b0m(char_index-1));
            else
                dpsi_res_0db(char_index) = (psi_res_0(char_index+1)-psi_res_0(char_index-1))/(b0m(char_index+1)-b0m(char_index-1));
   end
end
...
%                 end
    ...
    for iter = 1:6
        ...
        if iter ==1 && mainiter==1
            ...
            parfor j =1:length(b0m)
                v= zeros(size(l));
                tempchary = chary;
                tempcharz = charz;
                tempb1interp = b1interp;
                for i = 1:length(l)
                    ylev = tempchary(i,j) ;
                    zlev = tempcharz(i,j) ;
                    idy = find(chary(:,1)<=ylev,1,'last');
                    h = tempcharz(idy,1); % find thermocline depth
                    zint = (h:dz:zlev);
                    yint = ylev*ones(1,length(zint));
                    b1int = tempb1interp(yint,zint);
                    zint = [zint zlev+dz];
                    
                    v(i) =  nansum(b1int.*diff(zint));
                end
                P1(:,j) = v;
            end
            ...
            parfor j=1:length(b0m)
                ...
                for i=1:length(l)
                    ...
                    v(i) = -1/f*(tempP1interp(yi(2),zlev)-tempP1interp(yi(1),zlev))/(yi(2)-yi(1));
                end
                u1(:,j) = v;
            end
            ...
            parfor j = 1:length(b0m)
                ...
                for i=1:length(l)
                    ...
                    v(i) =  sqrt(-1)*kx*db0dz(i,j)*(nansum(P1int.*diff(zint)) + P1int(1)*(-hm) + b1int(1)*(-(hm^2)/2) )/2/ko/s0(i,j)/f;
                    u(i) = sqrt(-1)*kx*(nansum(u1int.*diff(zint)) + u1int(1)*(-hm) + b1y(idsurf)*(-(hm^2)/2));
        ...
        parfor char_index=1:length(b0m)
            ...
            %             if (id>2 & ~isempty(id))
            [T, Y] = ode23s(@(t, ysol) ord_eps(t,ysol, char_index,chary, dpsi_res_0db(char_index), l , y, f, ko,kx, db0dz, s0, u0, tau1, RHS1, RHS2) ...
                , l(1:id), [b1m(char_index) psi_res_1_ml(char_index)]);
            b1temp(char_index).Y = Y;
        end
        
        for char_index = 1:length(b0m)
            m = size(b1temp(char_index).Y,1);
            b1(1:m,char_index) = b1temp(char_index).Y(1:m,1);
            psi_res_1(1:m,char_index) = b1temp(char_index).Y(1:m,2);
%             disp(char_index)
        end
